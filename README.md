...
| 1 | 核心装配层（config / contracts / registry / llm / bootstrap）+ chat 样板模块 + CLI 入口 | ⬚ |
...
├── .env.example             # 配置契约（阶段 1 起由 core/config.py 消费）
├── src/agent_base/          # 基座源码（阶段 1 起填充 core/modules/extensions/entrypoints）
├── tests/                   # 测试（阶段 0 起即有冒烟测试）
├── docs/adr/                 # 架构决策记录
